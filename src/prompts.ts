...
import {
  buildSystemPrompt as buildSystemPromptShared,
  stripEmojiForPrompt as stripEmojiForPromptShared,
  formatRecentChat as formatRecentChatShared,
  formatEmojiChoices as formatEmojiChoicesShared,
  formatDiscoveryFindings as formatDiscoveryFindingsShared,
  formatWebSearchFindings as formatWebSearchFindingsShared,
  formatRecentLookupContext as formatRecentLookupContextShared,
  formatOpenArticleCandidates as formatOpenArticleCandidatesShared,
  formatVideoFindings as formatVideoFindingsShared,
  renderPromptMemoryFact as renderPromptMemoryFactShared,
  formatMemoryFacts as formatMemoryFactsShared,
  formatMemoryLookupResults as formatMemoryLookupResultsShared,
  formatImageLookupCandidates as formatImageLookupCandidatesShared,
  formatImageLookupResults as formatImageLookupResultsShared
} from "./prompts/promptFormatters.ts";
import {
  buildReplyPrompt as buildReplyPromptShared,
  buildAutomationPrompt as buildAutomationPromptShared,
  buildInitiativePrompt as buildInitiativePromptShared
} from "./prompts/promptText.ts";

function stripEmojiForPrompt(text) {
  return stripEmojiForPromptShared(text);
}

function formatRecentChat(messages) {
  return formatRecentChatShared(messages);
}

function formatEmojiChoices(emojiOptions) {
  return formatEmojiChoicesShared(emojiOptions);
}

function formatDiscoveryFindings(findings) {
  return formatDiscoveryFindingsShared(findings);
}

function formatWebSearchFindings(webSearch) {
  return formatWebSearchFindingsShared(webSearch);
}

function formatRecentLookupContext(recentWebLookups) {
  return formatRecentLookupContextShared(recentWebLookups);
}

function formatOpenArticleCandidates(candidates) {
  return formatOpenArticleCandidatesShared(candidates);
}

function formatVideoFindings(videoContext) {
  return formatVideoFindingsShared(videoContext);
}

function renderPromptMemoryFact(row, { includeType = true, includeProvenance = true } = {}) {
  return renderPromptMemoryFactShared(row, { includeType, includeProvenance });
}

function formatMemoryFacts(facts, { includeType = true, includeProvenance = true, maxItems = 12 } = {}) {
  return formatMemoryFactsShared(facts, { includeType, includeProvenance, maxItems });
}

function formatMemoryLookupResults(results) {
  return formatMemoryLookupResultsShared(results);
}

function formatImageLookupCandidates(candidates) {
  return formatImageLookupCandidatesShared(candidates);
}

function formatImageLookupResults(results) {
  return formatImageLookupResultsShared(results);
}
...
export function buildSystemPrompt(settings) {
  return buildSystemPromptShared(settings);
}

export function buildReplyPrompt(options) {
  return buildReplyPromptShared(options);
}

export function buildAutomationPrompt(options) {
  return buildAutomationPromptShared(options);
}
...
export function buildInitiativePrompt(options) {
  return buildInitiativePromptShared(options);
}
