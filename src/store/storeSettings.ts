...
import { decorateRuntimeSettings } from "../settings/runtimeCompat.ts";
...
if (normalizedJson === String(rawValue || "")) return decorateRuntimeSettings(normalized);

store.db
  .prepare("UPDATE settings SET value = ?, updated_at = ? WHERE key = ?")
  .run(normalizedJson, nowIso(), SETTINGS_KEY);
return decorateRuntimeSettings(normalized);
}
...
return decorateRuntimeSettings(normalizeSettings(parsed));
}
