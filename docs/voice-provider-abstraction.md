...
|---|---|---|
| Transcription method | `agentStack.runtimeConfig.voice.openaiRealtime.transcriptionMethod` | `"realtime_bridge"` |
| ASR model | `agentStack.runtimeConfig.voice.openaiRealtime.inputTranscriptionModel` | `"gpt-4o-mini-transcribe"` |
| Per-user bridge | `agentStack.runtimeConfig.voice.openaiRealtime.usePerUserAsrBridge` | `true` |
| Language mode | `voice.transcription.languageMode` | `"auto"` |
| Language hint | `voice.transcription.languageHint` | `"en"` |
...
| Setting | Key Path | Default |
|---|---|---|
| Transcription model | `agentStack.runtimeConfig.voice.sttPipeline.transcriptionModel` | `"gpt-4o-mini-transcribe"` |
...
When `voice.admission.mode == "classifier_gate"` and the turn survived deterministic gates, `runVoiceReplyClassifier()` makes a YES/NO call.

| Setting | Key Path | Default |
|---|---|---|
| Classifier | `agentStack.overrides.voiceAdmissionClassifier` | `"anthropic" / "claude-haiku-4-5"` |
| Admission mode | `voice.admission.mode` | `"classifier_gate"` |
| Music wake latch | `voice.admission.musicWakeLatchSeconds` | `15` |
...
| Setting | Key Path | Default |
|---|---|---|
| Provider | `agentStack.runtimeConfig.voice.generation.model.provider` | `"anthropic"` |
| Model | `agentStack.runtimeConfig.voice.generation.model.model` | `"claude-haiku-4-5"` |
...
| Setting | Key Path | Default |
|---|---|---|
| TTS model | `agentStack.runtimeConfig.voice.sttPipeline.ttsModel` | `"gpt-4o-mini-tts"` |
| TTS voice | `agentStack.runtimeConfig.voice.sttPipeline.ttsVoice` | `"alloy"` |
| TTS speed | `agentStack.runtimeConfig.voice.sttPipeline.ttsSpeed` | `1` |
...
| Setting | Key Path | Default |
|---|---|---|
| Enabled | `initiative.voice.enabled` | `false` |
| Provider | `initiative.voice.execution.model.provider` | `"anthropic"` |
| Model | `initiative.voice.execution.model.model` | `"claude-sonnet-4-6"` |
| Temperature | `initiative.voice.execution.temperature` | `1` |
| Eagerness | `initiative.voice.eagerness` | `50` |
| Min silence | `initiative.voice.minSilenceSeconds` | `45` |
| Min interval | `initiative.voice.minSecondsBetweenThoughts` | `60` |
...
|---|---|---|---|
| Reply path | `voice.conversationPolicy.replyPath` | `"native" \| "bridge" \| "brain"` | `"brain"` |
| Reply eagerness | `voice.conversationPolicy.replyEagerness` | `0-100` | `50` |
| Command-only mode | `voice.conversationPolicy.commandOnlyMode` | `boolean` | `false` |
...
| Eagerness | `initiative.voice.eagerness` | `50` |
| Min silence | `initiative.voice.minSilenceSeconds` | `45` |
| Min between thoughts | `initiative.voice.minSecondsBetweenThoughts` | `60` |

### ASR & STT

| Setting | Key Path | Default |
|---|---|---|
| ASR model (bridge) | `agentStack.runtimeConfig.voice.openaiRealtime.inputTranscriptionModel` | `"gpt-4o-mini-transcribe"` |
| Per-user bridge | `agentStack.runtimeConfig.voice.openaiRealtime.usePerUserAsrBridge` | `true` |
| Transcription method | `agentStack.runtimeConfig.voice.openaiRealtime.transcriptionMethod` | `"realtime_bridge"` |
| Language mode | `voice.transcription.languageMode` | `"auto"` |
| Language hint | `voice.transcription.languageHint` | `"en"` |
| STT model (brain) | `agentStack.runtimeConfig.voice.sttPipeline.transcriptionModel` | `"gpt-4o-mini-transcribe"` |
| TTS model | `agentStack.runtimeConfig.voice.sttPipeline.ttsModel` | `"gpt-4o-mini-tts"` |
| TTS voice | `agentStack.runtimeConfig.voice.sttPipeline.ttsVoice` | `"alloy"` |
| TTS speed | `agentStack.runtimeConfig.voice.sttPipeline.ttsSpeed` | `1` |
...
| Setting | Key Path | Default |
|---|---|---|
| Max session minutes | `voice.sessionLimits.maxSessionMinutes` | `30` |
| Inactivity leave | `voice.sessionLimits.inactivityLeaveSeconds` | `300` |
