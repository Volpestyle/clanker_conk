...
function getDocumentText(doc: Document) {
  return String(doc.body.textContent || "")
    .replace(/\s+/g, " ")
    .trim();
}

function hasText(doc: Document, value: string) {
  return getDocumentText(doc).includes(value);
}

function hasControl(doc: Document, id: string) {
  return Boolean(doc.querySelector(`#${id}`));
}
